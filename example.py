...
class MLP(object):
    ...
    def __init__(self, **kwargs):
        ...
        self.layers = kwargs['layers'] #[1,5,5,5,1]
        print("==========================")
        print(self.layers)
        ...
        for i in range(1, len(self.layers)):
            ...
            # starts: 0, middle point: 0.5, matrix: 1,5 -> 5, 5 -> 5, 5 -> 5,1
            print("self.layers[{}],self.layers[{}]".format(i-1, i), self.layers[i-1],self.layers[i],"\n" )
            self.bias.append(np.random.normal(0, 0.5, self.layers[i]))

            if i != len(self.layers) -1:
                self.activate.append(MLP.sigmoid)
            else:
                self.activate.append(MLP.identity) #last don't use sigmoid
...
if __name__ == '__main__':
    ...
    train_x = np.linspace(-5,5,100) #-5 ~5 dividing 100 evenly
    print(train_x)
